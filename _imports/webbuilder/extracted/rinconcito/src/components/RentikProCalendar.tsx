...
export const RentikProCalendar: React.FC<RentikProCalendarProps> = ({
    ...
    const fetchWindow = useCallback(async (windowFrom: Date, windowTo: Date) => {
        ...
        setError(null);
        try {
            const result = await fetchAvailability(fromStr, toStr);
            ...
            if (result.data) {
                const aptData = result.data.apartments.find(a => a.apartmentSlug === apartmentSlug);
                if (aptData) {
                    ...
                    for (const day of aptData.days) {
                        ...
                        const existing = cache.current.get(mk)!;
                        if (!existing.some(d => d.date === day.date)) {
                            existing.push(day);
                        }
                    }
                }
            ...
            // If it's demo mode, maybe show a hint if we want, but usually it's fine
            if (result.isDemo && !result.error) {
                console.log('[RentikPro] Calendar running in demo mode');
            }
    ...
    const daySize = compact ? 'h-9 w-9 text-xs' : 'h-12 w-12 md:h-14 md:w-14 text-sm';
        ...
        <div className={`bg-white rounded-3xl shadow-xl overflow-hidden border border-stone-100 ${className}`}>
            ...
            <div className={`bg-stone-900 ${headPad} flex justify-between items-center text-white`}>
                <span className="text-[10px] uppercase tracking-widest font-black opacity-80">
                    {t('availability.realtime_rentikpro')}
                </span>
            ...
            <div className={pad}>
                ...
                {loading && (
                    <div className="flex items-center justify-center gap-2 text-stone-400 py-2 mb-2">
                        ...
                        <span className="text-xs uppercase tracking-widest">{t('common.loading', 'Cargando...')}</span>
                    </div>
                )}
                        ...
                        return (
                            ...
                            >
                                <div className="flex flex-col items-center">
                                    <span className={sel === 'start' || sel === 'end' ? 'font-bold' : ''}>
                                        {dayNum}
                                    </span>
                                    {data?.price && !isPast && (
                                        <span className={`text-[8px] mt-0.5 font-bold ${sel === 'start' || sel === 'end' ? 'text-orange-200' : 'text-stone-400'
                                            }`}>
                                            {data.price}€
                                        </span>
                                    )}
                    ...
                    <div className="mt-4 pt-4 border-t border-stone-100">
                        <div className="bg-orange-50 rounded-2xl p-4 border border-orange-100">
                            <div className="flex items-center justify-between mb-3">
                                <div>
                                    <p className="text-xs font-black uppercase tracking-widest text-orange-700 mb-1">{t('common.your_selection', 'Selección')}</p>
