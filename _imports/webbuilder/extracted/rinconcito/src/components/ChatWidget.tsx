...
const QUICK_REPLIES = [
    ...
    { label: "Zonas de baño", query: "zonas de baño" },
    { label: "Astroturismo", query: "estrellas" },
    { label: "Bike Park", query: "bike park" },
...
export const ChatWidget: React.FC = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        {
            id: 'welcome',
            sender: 'bot',
            text: t('chat.welcome', 'Hola 👋 Soy el asistente del Rinconcito Matarraña. ¿Te ayudo con disponibilidad, actividades o dudas?')
        }
    ]);
    ...
    const processQuery = async (query: string) => {
        ...
        setTimeout(() => {
            const response = ChatEngine.analyze(query);
            setIsTyping(false);
        ...
        if (success) {
            ...
            setTimeout(() => {
                ...
                setMessages(prev => [...prev, {
                    id: Date.now().toString(),
                    sender: 'bot',
                    text: t('chat.success_title') + ' ' + t('chat.success_desc', 'Te responderemos lo antes posible.')
                }]);
            }, 3000);
        }
    };
        ...
        <div className="fixed bottom-0 right-0 md:bottom-6 md:right-6 z-[9999] w-full md:w-96 h-[80vh] md:h-[600px] bg-white md:rounded-[2rem] shadow-2xl flex flex-col border border-stone-200 overflow-hidden font-sans animate-in slide-in-from-bottom-10 fade-in duration-300">
            ...
            <div className="bg-stone-900 text-white p-6 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <span className="text-[10px] uppercase font-black tracking-widest text-stone-400">{t('intro.label', 'El Rinconcito')}</span>
                    </div>
                </div>
            ...
            {viewState === 'CHAT' ? (
                ...
                <div className="flex-1 p-8 bg-stone-50 overflow-y-auto">
                    ...
                    <h4 className="font-serif text-2xl text-stone-900 mb-2">{t('common.send_inquiry')}</h4>
                    <p className="text-sm text-stone-500 mb-8">{t('chat.form_desc')}</p>

                    <form onSubmit={handleFormSubmit} className="space-y-6">
                        <div>
                            <label className="block text-[10px] font-black uppercase tracking-widest text-stone-400 mb-2">{t('chat.name_label')}</label>
                            <input
                                type="text"
                                value={formData.name}
                        ...
                        <div>
                            <label className="block text-[10px] font-black uppercase tracking-widest text-stone-400 mb-2">{t('chat.contact_label')}</label>
                            <input
                                required
                                type="text"
                                ...
                                className="w-full bg-white border border-stone-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-orange-200"
                                placeholder={t('chat.contact_placeholder', 'ejemplo@email.com')}
                            />
                        </div>
                        <div>
                            <label className="block text-[10px] font-black uppercase tracking-widest text-stone-400 mb-2">{t('chat.message_label')}</label>
                            <textarea
                                required
                                rows={3}
                                ...
                                className="w-full bg-white border border-stone-200 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-orange-200"
                                placeholder={t('contact.form.help_label', '¿En qué te podemos ayudar?')}
                            />
                        </div>
                        <button
                            type="submit"
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 bg-stone-50 text-center">
                    ...
                    <h4 className="font-serif text-2xl text-stone-900 mb-2">{t('chat.success_title')}</h4>
                    <p className="text-sm text-stone-500">{t('chat.success_desc')}</p>
                </div>
            )}
        </div>
    );
