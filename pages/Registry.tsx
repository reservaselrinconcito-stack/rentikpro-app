...
import { 
  ...
  ExternalLink, CheckCircle2, Save, X, Plus, Clock, Search, ShieldCheck
} from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in pb-20">
      ...
      {activeTab === 'UNITS' && (
        <div className="grid grid-cols-1 gap-4">
           {registryUnits.map(unit => {
              const apt = apartments.find(a => a.id === unit.apartment_id);
              const prop = properties.find(p => p.id === apt?.property_id);
              ...
              return (
                 <div key={unit.id} className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row gap-6 items-start md:items-center">
                    <div className="flex-1">
                       <div className="flex items-center gap-2 mb-1">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${statusColor}`}>{unit.estado_tramitacion.replace('_', ' ')}</span>
                          <span className="text-xs font-bold text-slate-400">{unit.identificador_registral ? `CRU: ${unit.identificador_registral}` : 'Sin CRU'}</span>
                       </div>
                       <h3 className="text-xl font-black text-slate-800">{apt?.name}</h3>
                       <p className="text-sm text-slate-500 font-medium">{prop?.name} • {unit.direccion_completa || 'Dirección pendiente'}</p>
                       
                       <div className="flex gap-4 mt-4 text-xs font-bold text-slate-400">
                          <span className={unit.referencia_catastral ? 'text-emerald-600' : ''}>
                             {unit.referencia_catastral ? '✓ Ref. Catastral' : '⚠ Falta Ref. Catastral'}
                          </span>
                          <span className={unit.licencia_turistica ? 'text-emerald-600' : ''}>
                             {unit.licencia_turistica ? '✓ Licencia' : '⚠ Falta Licencia'}
                          </span>
                       </div>
                    </div>
                    
                    <div className="flex flex-col gap-2 min-w-[200px]">
                       <button onClick={() => setEditingUnit(unit)} className="px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-700 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors">
                          <FileText size={14}/> Editar Datos Registrales
                       </button>
                       <button 
                         onClick={() => window.open('https://sede.registradores.org/site/invitado/propiedad/busqueda/referencia-catastral', '_blank')}
                         className="px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-xl font-bold text-xs flex items-center justify-center gap-2 transition-colors"
                       >
                          <ExternalLink size={14}/> Trámite Online (Sede)
                       </button>
                    </div>
                 </div>
              );
