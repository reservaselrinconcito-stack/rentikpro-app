   ...
   async pullReservations(conn: ChannelConnection): Promise<SyncResult> {
      ...
      if (conn.force_direct) {
         ...
         try {
            ...
            // AUTO-FALLBACK: Si proxy retorna 403 (Host not allowed), usar conexión directa
            if (response.status === 403) {
               const errorText = await response.text();
               if (errorText.includes('Host not allowed')) {
                  console.warn(`[iCal] Proxy bloqueó host, intentando conexión directa para ${conn.alias}...`);
                  try {
                     response = await fetch(conn.ical_url, { headers });
                     usedProxy = false;
                     console.log(`[iCal] ✅ Conexión directa exitosa para ${conn.alias}`);
                  } catch (directErr: any) {
                     throw new Error(`PROXY bloqueó host y conexión directa falló: ${directErr.message}. Activa 'Forzar conexión directa' para este canal.`);
                  }
               }
            }
         } catch (proxyErr: any) {
            // Fallback adicional: si error de red, intentar directo
            if (!proxyErr.message || !proxyErr.message.includes('PROXY bloqueó')) {
               console.warn(`[iCal] Error de proxy, intentando conexión directa como fallback...`);
               try {
                  response = await fetch(conn.ical_url, { headers });
                  usedProxy = false;
                  console.log(`[iCal] ✅ Fallback a conexión directa exitoso`);
               } catch (directErr: any) {
                  throw new Error(`PROXY ERROR: ${proxyErr.message}. Fallback directo también falló: ${directErr.message}`);
               }
            } else {
               throw proxyErr;
            }
