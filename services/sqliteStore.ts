...
export class SQLiteStore implements IDataStore {
  ...
  private schemaFlags = {
    ...
    bookings_commission_fields: false,
  ...
  private async detectSchema() {
    try {
      ...
      this.schemaFlags.bookings_commission_fields = bInfo.some((c: any) => c.name === 'commission_override') && bInfo.some((c: any) => c.name === 'commission_type_override');
  ...
  async saveBooking(b: Booking): Promise<void> {
    ...
    if (this.schemaFlags.bookings_commission_fields) {
      columns.push('commission_override', 'commission_type_override');
      values.push(b.commission_override ?? null, b.commission_type_override ?? null);
    }
